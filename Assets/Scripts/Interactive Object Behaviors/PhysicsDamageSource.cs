...
public class PhysicsDamageSource : MonoBehaviour
{
    ...
    void OnCollisionEnter(Collision collision)
    {
        ...
        if (rb != null)
        {
            if (onlyDamageIfThisObjectIsMoving && rb.linearVelocity.magnitude == 0)
            {
        ...
        if (impactForce >= minimumVelocityForDamage)
        {
            ...
            if (bo != null)
                bo.Damage((int)(impactForce * _rb.mass * damageMultiplier), signalTypes.physics,rb.linearVelocity);
        }
